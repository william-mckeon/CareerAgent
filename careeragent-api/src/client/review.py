#!/usr/bin/env python3
# ============================================================================
# careeragent-api - Review client (outbound to careeragent-review)
# ============================================================================
#
# Backs the agent's `review_repos` tool. Modeled on DossierClient: owns its own
# httpx.AsyncClient with X-API-Key pre-attached, never raises on HTTP status
# (returns (status, body) so dispatch formats it). A repo-review fan-out can
# take MINUTES, so the read timeout is generous (unlike DossierClient's 15s).
# ============================================================================
...
class ReviewClient:
    ...
    async def review_repos(
        ...
        """POST /review-batch. Returns (status, body). Never raises."""
        if self._http is None:
