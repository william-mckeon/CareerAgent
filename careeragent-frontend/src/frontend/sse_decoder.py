#!/usr/bin/env python3
# ============================================================================
# careeragent-frontend - SSE Decoder
# Maintainer: William McKeon
# ============================================================================
#
# DESCRIPTION:
#   Standalone Server-Sent Events decoder for the careeragent-frontend Streamlit
#   application. This module owns the parsing of the SSE stream that flows
#   from careeragent-api back to the frontend on every /chat request.
#
#   The careeragent-api gateway forwards careeragent-infra's stream byte-for-byte,
#   and careeragent-infra in turn forwards the provider's stream byte-for-byte.
...
#     ▼
#   raw byte stream from careeragent-api
#     │
...
# ============================================================================
# Use a child logger of "careeragent.frontend" so log output unifies cleanly
...
logger = logging.getLogger("careeragent.frontend.sse_decoder")
...
# retry:) are valid SSE fields but are not used by careeragent-api or
# careeragent-infra.
...
# JSON-encoded ChatCompletion chunks. Both originate at careeragent-api and are
# documented in the careeragent-api datasheet's "Inbound HTTP Contracts"
# section.

# End-of-stream marker. Forwarded by careeragent-api when careeragent-infra closes
# the upstream stream cleanly. Always the final event in a successful
# response.
DONE_SENTINEL = "[DONE]"

# Mid-stream error marker. Emitted by careeragent-api when the upstream
...
KIND_ERROR = "error"
KIND_DONE = "done"
# P4 interactive channel: the coach paused to ask the user (or request an
# approval). Carried on a namespaced frame with no OpenAI `choices`.
KIND_SUSPEND = "suspend"

# P7 #19 typed structured streaming: richer progress events on the SAME namespaced
# `careeragent` frame (emitted ALONGSIDE the plain-text reasoning line, so they are
# purely additive — an un-upgraded decoder just ignores them). The event's payload
# rides SSEEvent.typed.
KIND_TOOL_START = "tool_start"       # typed = {name, args}
KIND_TOOL_RESULT = "tool_result"     # typed = {name, ok}
KIND_PLAN_UPDATE = "plan_update"     # typed = {plan: [{content, status}, ...]}
KIND_STEP = "step"                   # typed = {text}
# P7 #16 artifact frame: the coach rendered a downloadable résumé document. Rides
# the SAME namespaced `careeragent` frame; carries only METADATA (never the bytes)
# so the UI can show a download button that fetches the file from the download
# proxy by id. typed = {artifact_id, application_id, format, filename, bytes}.
KIND_ARTIFACT = "artifact"
_TYPED_EVENTS = frozenset({
    KIND_TOOL_START, KIND_TOOL_RESULT, KIND_PLAN_UPDATE, KIND_STEP, KIND_ARTIFACT,
})
...
class SSEEvent:
    ...
    Each kind populates a different subset of the remaining fields:
        ...
        kind="suspend"    →  pending  carries the paused run's question/approval
                             request {call_id, kind, question, options}
    ...
    Attributes:
        ...
        finish_reason:  Completion reason from the upstream model. Always
                        populated on "finish" events. Also populated on a
                        "reasoning"/"content" event when the upstream put a
                        token and the finish_reason in the SAME chunk — in that
                        case decode_sse_stream also emits a synthetic "finish"
                        event carrying it, so consumers that branch only on
                        "finish" still observe it. Empty string otherwise.
    ...
    # Populated for kind="suspend" only: the paused run's pending request the UI
    # renders — {call_id, kind: "question"|"approval", question, options[]}.
    pending: Optional[dict] = None
    # Populated for the P7 #19 typed kinds (tool_start/tool_result/plan_update/step):
    # the raw `careeragent` frame body, e.g. {"event": "plan_update", "plan": [...]}.
...
def parse_chunk(line: str) -> Optional[SSEEvent]:
    ...
    Notes:
        - The gate accepts the spec-legal "data:" prefix with OR without the
          trailing space. careeragent-api / careeragent-infra / the provider emit
    ...
    # field prefixes (event:, id:, retry:) but careeragent-api does not emit
    ...
    # ------------------------------------------------------------------
    # careeragent-api forwards careeragent-infra's [DONE] byte-for-byte. This is
    ...
    # ------------------------------------------------------------------
    # careeragent-api emits this when the upstream connection fails after SSE
    ...
    # ------------------------------------------------------------------
    # P4 SUSPEND FRAME — the coach paused to ask the user
    # ------------------------------------------------------------------
    # A namespaced frame with NO OpenAI `choices`:
    #   {"careeragent": {"event": "suspend", "pending_call_id": ...,
    #                    "pending_kind": "question"|"approval",
    #                    "payload": {"question": ..., "options": [...]}}}
    # Emitted by careeragent-api, persisted by careeragent-sessions; the frontend
    # renders the question/approval and POSTs the answer to resume. Detected here
    # BEFORE the choices check (which would otherwise skip it as shapeless).
    if isinstance(chunk, dict):
        ca = chunk.get("careeragent")
        if isinstance(ca, dict):
            event = ca.get("event")
            if event == "suspend" and ca.get("pending_call_id"):
                body = ca.get("payload") or {}
                if not isinstance(body, dict):
                    body = {}
                options = body.get("options")
                return SSEEvent(kind=KIND_SUSPEND, pending={
                    "call_id": str(ca.get("pending_call_id")),
                    "kind": str(ca.get("pending_kind") or "question"),
                    "question": str(body.get("question") or ""),
                    "options": [str(o) for o in options] if isinstance(options, list) else [],
                    # Full payload carried through for richer pending kinds (P7 #20
                    # plan_proposal = {summary, steps}); the UI sanitizes on render.
                    "payload": body,
                })
            # P7 #19 typed progress frames. An UNKNOWN careeragent event is left to
            # fall through to the "no choices" path below and is ignored (forward-compat).
            if event in _TYPED_EVENTS:
                return SSEEvent(kind=str(event), typed=ca)
    ...
    # If a token and a finish_reason arrive in the SAME chunk (the OpenAI spec
    # permits e.g. a final content token co-located with finish_reason="length"),
    # carry the finish_reason on the token event so it is not lost when we return
    # early at the token branch below. decode_sse_stream emits a synthetic
    # KIND_FINISH from it, so consumers that only inspect KIND_FINISH still see a
    # truncation. Empty string when the chunk carried no finish_reason.
    colocated_finish = str(finish_reason) if finish_reason else ""
    ...
    if reasoning_token is not None:
        if isinstance(reasoning_token, str) and reasoning_token:
            return SSEEvent(
                ...
                finish_reason=colocated_finish,
            )
...
def decode_sse_stream(
    ...
    Termination:
        ...
        cleanup will close the underlying iterator. careeragent-api's own
    ...
    try:
        for line in lines:
            ...
            # A single chunk can carry BOTH a token and a finish_reason (e.g. a
            # final content token co-located with finish_reason="length").
            # parse_chunk returns the token event with finish_reason populated;
            # emit a synthetic terminal event after it so a truncation is never
            # silently dropped and consumers that only inspect KIND_FINISH still
            # observe it.
            if event.kind in (KIND_REASONING, KIND_CONTENT) and event.finish_reason:
                saw_finish = True
                logger.debug(
                    f"SSE synthetic finish from co-located chunk: "
                    f"finish_reason={event.finish_reason}"
                )
                yield SSEEvent(kind=KIND_FINISH, finish_reason=event.finish_reason)
